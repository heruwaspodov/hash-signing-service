...
// Compatible with SoftHSM2 (dev), AWS CloudHSM, Google Cloud HSM, Azure Managed HSM.
...
// opens a session, logs in, and locates the private key by label.
...
func NewHSMSigner(modulePath, tokenLabel, pin, keyLabel string) (*HSMSigner, error) {
	...
	// Locate the private key by its CKA_LABEL.
	...
	if len(objects) == 0 {
		return nil, fmt.Errorf("private key with label %q not found in HSM", keyLabel)
	}
...
// performs a raw RSA operation. The caller must prepend the ASN.1 DigestInfo prefix
// before passing the data to C_Sign — otherwise the resulting signature will be
// structurally different from a Go stdlib signature and PDF validators will reject it.
